...
const Navbar = () => {
  ...
  const navItems: NavItemProps[] = [
    { href: "#home", text: "Home" },
    ...(isLoggedIn ? [{ href: "#leaderboard", text: "Leaderboard" }] : []),
  ...
  return (
    <header className="sticky inset-x-0 top-0 z-50 py-6 bg-body">
      <Container>
        <nav className="w-full flex justify-between gap-6 relative items-center">
          ...
          <div className="sm:hidden flex bg-body items-center justify-end w-full relative">
            ...
            {dropdownOpen && (
              <div
                className={`absolute top-full left-0 mt-2 w-full bg-body border border-box-border rounded shadow-md z-50 transition-transform ease-in-out ${
                  dropdownOpen
                    ? "duration-300 opacity-100 scale-100"
                    : "duration-500 opacity-0 scale-95"
                }`}
              >
                  ...
                  {navItems.map((item, key) => (
                    <li key={key}>
                      <a
                        ...
                        className={`block w-full text-heading-1 border border-color text-left px-4 py-3 hover:bg-gray-400 ${
                          activeItem.href === item.href ? "font-bold" : ""
                        }`}
                      >
                        {item.text}
                      </a>
